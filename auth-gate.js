...
function overlay(){
 ...
 el.innerHTML=`<section class="wrv-auth-visual">
   <div class="wrv-auth-brand"><div class="wrv-auth-logo">Work Ready Vault</div><small>Executive Leadership Coaching · A division of Foundations Counselling Academy</small></div>
...
function isEmptyPayload(payload){return !Object.keys(payload.elrpState||{}).length&&!Object.keys(payload.elrpDailyHabits||{}).length}

async function push(force=false){
 if(syncing||!cloudAvailable)return;
 ...
 try{
   ...
   if(r.ok&&out.cloudAvailable===false){cloudAvailable=false;lastSnapshot=current;markSync('Local only')}
   else if(r.ok){lastSnapshot=current;markSync('Synced')}
   else{console.error('WRV sync failed',r.status,out);markSync('Sync error')}
 }catch(err){console.error('WRV sync failed',err);markSync('Sync error')}
...
async function activateClient(el){
 try{
   ...
   if(r.ok&&out.cloudAvailable===false){cloudAvailable=false;markSync('Local only')}
   else if(!r.ok)throw new Error(out.error||'Unable to load cloud data');
   if(r.ok&&out.cloudAvailable!==false&&out.data){
     const server=out.data;
     const local=readLocal();
     const serverPayload={elrpState:server.elrpState||{},elrpDailyHabits:server.elrpDailyHabits||{}};
...
async function boot(){
 const gate=overlay();
 try{
   if(await establishPortalSessionFromSupabase()){
