...
function enhanceWeek(key){
 ...
 card.innerHTML=`<div class="working-notes-head"><div><span class="eyebrow dark">02 · Working Notes</span><h2>${esc(cfg.title)}</h2><p>${esc(cfg.prompt)}</p></div><span class="working-notes-status">Autosaves as you type</span></div><div class="working-notes-cue"><strong>Working prompt</strong><span>${esc(cfg.cue)}</span></div><label class="working-notes-field"><span>Your notes</span><textarea data-toolfield="${key}-working-notes" maxlength="2500" placeholder="Capture key ideas, examples, questions or coaching notes here…">${esc(saved)}</textarea><small><span data-note-count>0</span>/2500 characters · Bring these notes into your live coaching session.</small></label>`;
